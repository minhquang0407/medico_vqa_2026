...
class SinkhornOTRouter(nn.Module):
    ...
    def forward(
        ...
        # Force float32 computation inside Sinkhorn OT to prevent underflow in float16 autocast
        with torch.autocast(device_type=visual_tokens.device.type, enabled=False):
            ...
            if visual_target_mass is not None:
                visual_target_mass = visual_target_mass.float()
            ...
            if not return_diagnostics:
                return aligned_visual.to(dtype=orig_dtype)

            return {
                "transport_plan": transport_plan.to(dtype=orig_dtype),
                "cost_matrix": cost_matrix.to(dtype=orig_dtype),
                "aligned_visual": aligned_visual.to(dtype=orig_dtype),
                "ot_cost": ot_cost.to(dtype=orig_dtype),
                "per_sample_ot_cost": per_sample_cost.to(dtype=orig_dtype),
                "transport_entropy": self.transport_entropy(transport_plan).to(dtype=orig_dtype),
                "text_mass": transport_plan.sum(dim=-1).to(dtype=orig_dtype),
                "visual_mass": transport_plan.sum(dim=1).to(dtype=orig_dtype),
                "visual_target_mass": visual_target_mass.to(dtype=orig_dtype) if visual_target_mass is not None else None,
            }
